...
# (V2.2 - Final & Fully Standardized)
# เวอร์ชันสุดท้ายที่ได้มาตรฐานเดียวกับ Gemini ทุกประการ
...
from groq import Groq, RateLimitError, APIStatusError, AuthenticationError
...
class KnowledgeGraphExtractorLlama:
    """
    โรงงานสกัดความรู้สำหรับ "Llama 3 on Groq"
...
**[คำสั่งเด็ดขาด]**
คุณต้องตอบกลับเป็น JSON object ที่ถูกต้องและสมบูรณ์เพียงอย่างเดียวเท่านั้น
**ห้าม** ใส่ข้อความอธิบาย, คำเกริ่นนำ, คำลงท้าย, หรือ markdown formatting (เช่น ```json) ใดๆ ทั้งสิ้น
คำตอบของคุณต้องเริ่มต้นด้วยเครื่องหมาย `{` และจบลงด้วยเครื่องหมาย `}` เท่านั้น
...
"""
    ...
    # --- ฟังก์ชันที่แก้ไขใหม่ (มี try-except-retry ที่ถูกต้อง) ---
        ...
        for attempt in range(max_retries):
            try:
                ...
                if graph_data and isinstance(graph_data, dict) and "nodes" in graph_data:
                    ...
                    raise ValueError(f"Failed to extract valid JSON. Raw response: {raw_response_text[:200]}...")
            except AllGroqKeysOnCooldownError as e:
                raise e
            except RateLimitError as e:
                print(f"⏳ Groq Rate limit for key '...{api_key[-4:]}'. Trying next... ({attempt + 1}/{max_retries})")
                self.key_manager.report_failure(api_key, error_type='rate_limit')
                last_exception = e
            
            # !!! แก้ไข Block นี้ !!!
            except AuthenticationError as e:
                print(f"🚫 Invalid Groq key '...{api_key[-4:]}'. Deactivating permanently.")
                self.key_manager.report_failure(api_key, error_type='invalid_key')
                last_exception = e
            # !!! ----------------- !!!

            except APIStatusError as e:
                if e.status_code >= 500:
                    print(f"💥 Groq server error for key '...{api_key[-4:]}'. Trying next... ({attempt + 1}/{max_retries})")
                    self.key_manager.report_failure(api_key, error_type='server_error')
                    last_exception = e
                else:
                    raise e
            except Exception as e:
                print(f"💥 Unexpected error with Groq key '...{api_key[-4:]}': {e}. Trying next...")
                self.key_manager.report_failure(api_key, error_type='generic')
                last_exception = e
        raise last_exception if last_exception else Exception("Failed to process chunk with Groq after all retries.")
...
def main():
    ...
    print("--- 🏭 Starting Knowledge Graph Extraction Process (V2.2 - Llama Production) ---")
    ...
    try:
        ...
        extractor_instance = KnowledgeGraphExtractorLlama(
    ...
    # [FIX] ดักจับเฉพาะ Exception ของ Groq Key Manager เท่านั้น
    except AllGroqKeysOnCooldownError as e:
